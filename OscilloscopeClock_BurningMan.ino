/******************************************************************************
  ...
  * Significant Mods for )'( by SR 5/25 *
...
#include "DataTable.h"
#include "DataTableBM.h" // rotating BM logo
...
#define NTP
  ...
  // Something is fishy with the way that NTPtime treats the server name!
  // Works only after hard coding ntp1.lrz.de in NTPtimeESP.cpp - SR
  NTPtime NTPch("de.pool.ntp.org"); // Choose your server pool
  char *ssid      = "LanRouterH1";        // Set your WiFi SSID
  char *password  = "QuarksLeptons10";        // Set your WiFi password
...
int           Timeout        = 10;
const    long interval       = 1000; // my clock is correct! 990; //milliseconds, you should twick this
...
void setup() 
{
  ...
  Serial.println(")'( adaptation SR/gaijin 2025");
  //rtc_clk_cpu_freq_set(RTC_CPU_FREQ_240M);
//  Serial.println("CPU Clockspeed: ");
//  Serial.println(rtc_clk_cpu_freq_value(rtc_clk_apb_freq_get()));
//  Serial.println(esp_clk_cpu_freq());
  ...
  if (h > 12) h=h-12;
    ...
    if (Timeout!=0){
      Serial.println("\nWiFi connected");
      Serial.println("Sending NTP request to Server.");
...
void loop() {
  ...
  // SR
  PlotTable(SecPtrBM,sizeof(SecPtrBM),0x00,0,85*s);  // 85*s, each figure is 85 bytes 
  PlotTable(SRlogo,sizeof(SRlogo),0x00,1,0);      //2 to back trace
